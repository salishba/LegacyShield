        ...
        try:
            cursor.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS system_info (
                    ...
                    domain TEXT,
                    part_of_domain INTEGER DEFAULT 0,
                ...
                -- =====================================================
                -- PATCH STATE (DETERMINISTIC)
                -- =====================================================
                CREATE TABLE IF NOT EXISTS patch_state (
                    patch_state_id INTEGER PRIMARY KEY AUTOINCREMENT,
                    cve_id TEXT NOT NULL,
                    kb_id TEXT,
                    state TEXT CHECK(state IN ('INSTALLED','MISSING','MITIGATION_ONLY')),
...
def insert_installed_kb(db: RuntimeDatabaseManager, kb: dict, host_hash: str, scan_time: str):
    cursor = db.conn.cursor()
    cursor.execute("""
        INSERT OR REPLACE INTO installed_kbs (
            kb_id,
            install_date,
            source,
            host_hash,
